...
class VentoJavaScriptAttributeValueInjector : MultiHostInjector {
    private companion object {
        ...
        const val EVENT_MAGICS = "/** @type {eventType} */\nlet ${'$'}event;\n\n"
    }
    ...
    private fun getJavaScriptRanges(host: XmlAttributeValue, content: String): List<TextRange> {
        ...
        if (host.containingFile.viewProvider.languages.none { "PHP" == it.id || "Blade" == it.id }) {
    ...
    private fun addTypingToEventMagics(directive: String): String {
        val eventName = AttributeUtil.getEventNameFromDirective(directive)
        return EVENT_MAGICS.replace("eventType", eventName)
    }
}
